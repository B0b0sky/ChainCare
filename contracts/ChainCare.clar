...
(define-public (contribute-to-research (research-id uint))
  (let ((caller tx-sender)
        (patient-data (unwrap! (map-get? patients {patient-id: caller}) ERR_NOT_REGISTERED))
...
(define-public (claim-research-reward (research-id uint))
  (let ((caller tx-sender)
        (contribution (unwrap! (map-get? research-contributions {patient-id: caller, research-id: research-id}) ERR_NOT_AUTHORIZED)))
